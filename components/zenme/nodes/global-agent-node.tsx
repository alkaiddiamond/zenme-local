...
export function GlobalAgentNode({ data, id, selected }: NodeProps) {
  ...
  return <>
    <NodeFrame className="flex h-full w-full flex-col overflow-hidden" selected={Boolean(selected)}>
      ...
      <header className="flex items-center gap-3 border-b border-zinc-200 px-4 py-3"><Network className="size-4" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{nodeData.globalGoal || "历史 Global Agent"}</p><p className="text-[11px] text-zinc-500">{orchestration ? `${globalStatusLabel(orchestration.status)} · ${orchestration.tasks.length} 个 Sub-agent · 并发 ${orchestration.concurrencyLimit}` : "加载调度状态"}</p></div>{orchestration?.status === "running" ? <Loader2 className="size-4 animate-spin" /> : null}</header>
