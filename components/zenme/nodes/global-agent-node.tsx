...
import { AlertTriangle, Archive, Check, ChevronsDownUp, ChevronsUpDown, FileDiff, GitMerge, Loader2, Network, Pin, RotateCcw, Sparkles, Square } from "lucide-react";
import { useCallback, useEffect, useState } from "react";

import { ChangeSetDialog } from "@/components/zenme/change-set-dialog";
import { runGlobalOrchestration } from "@/components/zenme/canvas/global-agent-runner";
...
export function GlobalAgentNode({ data, id, selected }: NodeProps) {
  ...
  async function approve(taskId: string, executionId: string, commandId: string) {
    setBusy(commandId);
    try {
      await approveAgentCommandFromApi(projectId, executionId, commandId);
      await executeAgentWorkspaceToolFromApi({ projectId, executionId, name: "run_approved_command", arguments: { commandRequestId: commandId } });
      await runWorkspaceAgent({ projectId, executionId, model: nodeData.globalModel ?? "" });
      await updateGlobalOrchestrationFromApi({ action: "refresh", orchestrationId, projectId });
      await runGlobalOrchestration({ model: nodeData.globalModel ?? "", orchestrationId, projectId });
      await refresh();
    } catch (nextError) { setError(nextError instanceof Error ? nextError.message : "Sub-agent 命令执行失败"); }
    finally { setBusy(""); }
  }

  async function mutate(action: "stop" | "retry", subtaskId?: string) {
    setBusy(`${action}:${subtaskId ?? ""}`);
    try {
      await updateGlobalOrchestrationFromApi({ action, orchestrationId, projectId, subtaskId });
      if (action === "retry") await runGlobalOrchestration({ model: nodeData.globalModel ?? "", orchestrationId, projectId });
      await refresh();
  ...
  return <>
    <NodeFrame className="flex h-full w-full flex-col overflow-hidden" selected={Boolean(selected)}>
      ...
      {!folded ? <OverlayScrollArea className="min-h-0 flex-1" contentKey={orchestration?.updatedAt} viewportClassName="nodrag nowheel h-full overflow-auto p-4"><div className="space-y-2">{orchestration?.tasks.map((task, index) => {
        const detail = details[task.id]; const command = detail?.commandRequests.find((item) => item.status === "proposed"); const retryable = ["failed", "timedOut", "stopped", "interrupted"].includes(task.status);
        return <div className="rounded-lg border border-zinc-200 p-3 text-xs" key={task.id}><div className="flex items-center gap-2"><span className="flex size-5 items-center justify-center rounded-full bg-zinc-100 text-[10px]">{index + 1}</span><span className="font-medium">{task.title}</span><span className="ml-auto text-zinc-500">{subtaskStatusLabel(task.status)}</span></div><p className="mt-1 truncate text-[10px] text-zinc-500">{task.rootDisplayName ? `${task.rootDisplayName} · ` : ""}{task.allowedPathPrefixes.join(", ")}</p>{task.resultSummary ? <p className="mt-2 text-emerald-700">{task.resultSummary}</p> : null}{task.error ? <p className="mt-2 text-red-600">{task.error}</p> : null}{command && task.agentExecutionId ? <div className="mt-2 rounded bg-amber-50 p-2 text-amber-900"><code className="whitespace-pre-wrap break-all">{command.command ?? `${command.executable} ${command.args.join(" ")}`}</code><button className="mt-2 flex items-center gap-1 rounded bg-zinc-900 px-2 py-1 text-white" disabled={Boolean(busy)} onClick={() => void approve(task.id, task.agentExecutionId!, command.id)}><Check className="size-3" />批准并运行一次</button></div> : null}{retryable ? <button className="mt-2 flex items-center gap-1 rounded px-2 py-1 hover:bg-zinc-100" disabled={Boolean(busy)} onClick={() => void mutate("retry", task.id)}><RotateCcw className="size-3" />单独重试</button> : null}</div>;
