...
describe("rendered canvas nodes", () => {
  ...
  it("locks a source while one of its generation children is running", () => {
    ...
    expect(renderedNodes.find((item) => item.id === "text")?.data.hasRunningAgentTurn).toBe(true);
    expect(renderedNodes.find((item) => item.id === "image")?.data.hasRunningAgentTurn).toBe(true);
  });
  ...
  it("keeps archived execution nodes out of the main canvas and injects lifecycle controls into active results", () => {
    ...
    const archivedViewNodes = getRenderedCanvasNodes({
    ...
    expect(archivedViewNodes.map((item) => item.id)).toEqual(["archived"]);
    expect(archivedViewNodes[0].data.hasOutgoingEdge).toBe(false);
  });
