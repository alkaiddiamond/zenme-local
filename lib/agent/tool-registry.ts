...
const definitions = [
  ...
  define("list_directory", "列出指定目录的直接子项。relativePath 接受 Workspace 相对路径或本机绝对路径，可直接浏览 Workspace 外的目录，无需先绑定；绝对路径请求返回绝对路径。", "read", true, { relativePath: "." },
    (value) => isObject(value) && optionalString(value.relativePath) && optionalString(value.rootId)),
  define("glob_files", "按 Glob 路径模式查找文件，例如 **/*.ts。pathPrefix 可为本机绝对目录，直接搜索 Workspace 外并返回绝对路径；相对路径默认搜索所有可读 Workspace 根，rootId 可指定根。只返回路径。", "read", true, { pattern: "**/*.ts", pathPrefix: ".", maxResults: 200 },
  ...
  define("search_files", "搜索文本文件内容。pathPrefix 可为本机绝对目录，直接搜索 Workspace 外并返回绝对路径；相对路径默认搜索所有可读 Workspace 根，rootId 可指定根。跨根结果返回 rootId。", "read", true, { query: "symbol", pathPrefix: ".", maxResults: 50 },
  ...
  define("read_file", "读取文件，relativePath 接受 Workspace 相对路径或本机绝对路径，可直接读取 Workspace 外的文件，无需先绑定。文本按 startLine/endLine 分段；PDF 用 pages 读取指定页（例如 1-5 或 1,3,5-7，单次最多 20 页）；Workspace 图片使用 view_image。也可读取当前 Execution 返回的绝对 outputFilePath。不得循环读取轮询任务。", "read", true, { relativePath: "src/index.ts", startLine: 1, endLine: 400 },
