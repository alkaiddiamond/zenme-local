...
export function getProjectAgentPlanFilePath(projectId: string, dataDir: string) {
  return resolveInside(getProjectDir(projectId, dataDir), "agent", "plans", "PLAN.md");
}

export async function readProjectAgentPlan(projectId: string, dataDir: string) {
  try {
    return await fs.readFile(getProjectAgentPlanFilePath(projectId, dataDir), "utf8");
  } catch (error) {
...
export async function writeProjectAgentPlan(projectId: string, content: string, dataDir: string) {
  if (Buffer.byteLength(content, "utf8") > MAX_PLAN_BYTES) throw new Error("计划内容超过 1 MiB");
  const filePath = getProjectAgentPlanFilePath(projectId, dataDir);
