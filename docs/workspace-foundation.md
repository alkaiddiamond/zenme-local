...
文档状态：当前 Workspace 工程契约。

产品依据：相邻 `../zenme-doc/AI Project Workspace 开发指导.md`

## 用户结果与非目标

一个 Project 可以安全绑定、解除和重新关联主 Workspace，并在同一 Binding 中加入多个稳定 Root ID。每个根独立维护目录身份、状态、权限与 Git 摘要；用户能够看到绑定目录、信任状态、权限与 Git 基础状态。
...
3. 文件树与搜索默认遵守 `.gitignore`，并始终应用 Zenme 的敏感文件与生成目录排除规则。当前不提供覆盖这些安全排除的设置。
4. `.env`、私钥、证书、凭据数据库和常见密钥文件默认不得进入 Agent、索引或模型上下文。用户主动打开文件只授权该次本地查看，不改变 Agent 与索引的默认拒绝规则。
...
| File Document | `workspace/documents.json` | Workspace File 的共享身份、读取视图与编辑状态 |
| File Node | Canvas Snapshot | File Document 的画布视图；不得复制权威文件内容 |
| ChangeSet | `workspace/change-sets.json` | AI/批量文件修改、基线版本、审批、应用与回退状态；不得退化为聊天 Markdown |
| Execution | 既有 execution store | 一次实际执行及恢复状态；工具日志不进入 Canvas Snapshot |
| Project Memory | `memory/index.json` | 有来源、版本和失效状态的派生知识，不是文件真相源 |
...
| Capability | 绑定默认值 | 当前授权规则 |
| --- | --- | --- |
| `read` | `true` | 用户绑定并确认目录后允许 |
| `write` | `false` | 用户直接保存或批准 ChangeSet 后允许对应写入 |
| `delete` | `false` | 每次或按受控规则单独批准 |
| `execute` | `false` | 命令与工作目录受控批准 |
| `gitWrite` | `false` | 控制目标 Root 的本地 Git 写操作；远程 Git 操作仍逐次审批，破坏性操作直接拒绝 |
...
5. capability 变更使用独立动作与命名 IPC，不与绑定动作隐式合并。
...
canonical path + identity + git inspection
        ↓
user-selected external directory (capability gated)
```
...
真机列是发布候选需要重新执行的场景，不代表当前工作树已经通过；结果应记录在对应发布或变更审查中，而不是长期固化在本工程契约里。
