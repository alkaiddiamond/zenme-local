...
本文档记录 AI Project Workspace Phase 1 的文件身份与同步契约。Workspace 的授权、目录身份和能力边界见 [workspace-foundation.md](workspace-foundation.md)，Phase 2–3 的编辑与审批见 [editable-files-and-change-sets.md](editable-files-and-change-sets.md)。
...
- Phase 1 的读取边界不提供执行或 Git 写操作；后续保存与删除仍需独立 capability，并遵循 ChangeSet 契约。
...
- `.env*`、私钥、证书和常见凭据文件会标记为敏感。Phase 1 允许用户主动查看，但后续 Agent、索引和模型输入必须默认排除。
...
- `PUT /api/projects/:projectId/workspace/documents/:documentId`：Phase 2 以预期内容哈希原子保存。
