...
- 容器、编码探测与代理转码：现有独立音乐服务中的 `ffprobe` / `FFmpeg`
- 波形数据：预计算峰值；先验证 WaveSurfer 可直接消费的数据格式，再决定使用 `audiowaveform` 或服务端 PCM 降采样
- 播放会话：Zenme Local 自己实现很薄的全局 `AudioSession` 适配层，不自行实现播放器或波形引擎

不建议首版引入 Howler.js。WaveSurfer 已覆盖单轨播放、波形和结构区段，叠加另一套播放状态源没有收益。

产品当前以音乐分析和生成 Suno 提示词为核心，不建设多轨 DAW。`waveform-playlist` 与 Tone.js 的调研结论保留为远期开放方向，不进入当前实现范围。
...
播放器节点消费一条上游音乐连线，显示封面、信息、波形和播放控件，并作为歌词、综合分析和 Suno 提示词等分析节点的唯一父级。运行时状态包括：
...
播放器节点在画布内展示紧凑波形、基础音乐信息、总时长和播放控制，负责创建分析任务，并提供按需创建或定位各类分析子节点的入口。音乐节点不直接连接分析结果。

### 3.3 歌词与结构联动

播放器波形、歌词行和音乐结构区段共享统一秒制时间轴。播放器提供入口创建或定位歌词子节点；歌词节点按结构区段组织带时间戳歌词，播放时同步高亮，点击歌词或结构区段可反向控制播放位置。综合分析和 Suno 提示词也作为播放器子节点按需展开。
...
7. 验证歌词行、结构区段、播放头双向同步以及长歌词列表的滚动定位性能。
...
- 分析结果是在播放器波形上叠加，还是由独立“音乐分析结果节点”展示；建议播放器提供轻量叠加，详细结果保留独立节点。
- 是否允许在歌词节点直接修订歌词与时间戳，以及如何保留人工版本和模型版本。
